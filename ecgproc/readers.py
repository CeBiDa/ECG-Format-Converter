...
def read_mat(file_path):
    ...
    # Signals (n_leads, n_samples). Only transpose when the first axis is
    # clearly samples (large) and the second clearly leads (<=16); the old
    # shape[0]>shape[1] check transposed short 12-lead segments like (12,10).
    if signals.ndim != 2:
        raise ValueError(f"MAT signals must be 2D, got shape {signals.shape}")
    if signals.shape[0] > 16 and signals.shape[1] <= 16:
        signals = signals.T
...
def read_csv(file_path):
    ...
    # Header-less CSVs silently lose their first data row (it becomes the
    # header) and get numeric lead names that later zero-fill (see H3).
    # Fail loudly instead so the user can add a header row.
    try:
    ...
    if is_numeric_header:
        raise ValueError(f"CSV {file_path} looks header-less (columns parse as numbers); "
                         "add a lead-name header row (e.g. I,II,III,aVR,aVL,aVF,V1..V6).")
    meta = {
        "source": "csv",
...
def read_asc(file_path):
    ...
    try:
        ...
        is_numeric_header = False
    if is_numeric_header:
        raise ValueError(f"ASC {file_path} looks header-less (columns parse as numbers); "
                         "add a lead-name header row.")
...
def read_xml(file_path):
    ...
    with open(file_path,'r',encoding='utf-8',errors='ignore') as f:
        xml_text = f.read()
    # Tool's own generic output (<ECG><lead name=..>) is not a readable input
    # schema (see H4) -- say so instead of the generic rhythms error.
    if "<lead " in xml_text and "ECG_RHYTHMS" not in xml_text:
        raise ValueError(f"{file_path} looks like this tool's own generic XML output, "
                         "which is for downstream consumers and cannot be read back. "
                         "Convert from the original vendor XML / HL7 instead.")
    ...
    for chan in channels:
        ...
        if samples.size != TARGET_SAMPLES:
            # Truncated exports must not be FFT-stretched to the first lead's
            # length (that invents duration); pad short leads with zeros and
            # truncate long ones (see C4).
            samples=zero_pad(samples, TARGET_SAMPLES)
