...
def create_dicom_file(xml_file_path, output_folder, anonymize=False, override=False):
    ...
    try:
        ...
        stacked_waveforms = np.stack([lead_data[ld] for ld in FINAL_LEADS], axis=0)
        ...
        # build DICOM (12, N orientation; write_dcm no longer transposes short records)
        write_dcm(output_file_path, stacked_waveforms, meta)

        stats.count_success+=1
        # Return (N,12) for backward compat with convert.py fast-path transpose;
        # new callers should use meta/signal_names with (12,N).
        return stacked_waveforms.T, meta
    except Exception:
        stats.count_failed+=1
        return None,None
