...
def impute_missing(signal):
    # Work on a copy (callers pass row views) and treat any non-finite
    # (NaN or +/-inf) as missing; inf previously flowed into filters and
    # exploded in the HL7 writer (see H2/M5).
    ...
    if bad.any():
        ...
        sig[bad] = np.interp(np.flatnonzero(bad), np.flatnonzero(good), sig[good])
