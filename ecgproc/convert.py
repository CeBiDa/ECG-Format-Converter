...
from ecgproc.pacedetect import METHOD_NAME as PACE_METHOD, detect_pacemaker


def _pace_screen(signals_2d, fs, fs_source, log, options=None):
    """Run pre-filter pacemaker screening on raw signals.

    ``signals_2d`` is (n_leads, n_samples). Never raises: failures map to
    flag "unknown" so one bad record never aborts the batch.
    """
    try:
        res = detect_pacemaker(signals_2d, fs, options=options,
                               fs_source=fs_source)
    except Exception as exc:  # pragma: no cover - defensive, detect already guards
        _log = log if log is not None else tqdm.write
        _log(f"Pace-screen failed ({exc}); recording as unknown.")
        res = {"flag": "unknown", "score": 0.0, "n_spikes": 0,
               "fs_used": fs if fs else "", "fs_source": fs_source,
               "method": PACE_METHOD}
    return res


def _pace_summary_fields(res):
    flag = res.get("flag", "unknown")
    return {
        "pacedetect_flag": "" if flag == "unknown" else str(int(flag)),
        "pacedetect_score": res.get("score", 0.0),
        "pacedetect_n_spikes": res.get("n_spikes", 0),
        "pacedetect_fs": res.get("fs_used", ""),
        "pacedetect_fs_source": res.get("fs_source", ""),
        "pacedetect_method": res.get("method", PACE_METHOD),
    }
...
def process_file(file_path, output_dir, output_format, summary_list, pipeline, default_fs,
                 ...
                 pace_enabled=True, pace_options=None):
    _log = log if log is not None else tqdm.write
        ...
        if ext == ".mat":
            ...
            if output_format == "dcm" and not is_hl7 and not pipeline and not target_fs:
                try:
                    ...
                    failed_before = stats.count_failed
                    ...
                    if meta is not None:
                        ...
                        # Pre-filter pace screen on the raw vendor samples (fast path
                        # has no filtering either, so this is the right signal).
                        if pace_enabled:
                            _fs_fp = meta.get("fs") or default_fs
                            _src_fp = ("file" if meta.get("fs")
                                       else ("fallback" if default_fs else ""))
                            _sig_fp = (signals.T if signals is not None
                                       and getattr(signals, "ndim", 0) == 2 else signals)
                            if _sig_fp is not None and _fs_fp:
                                _res_fp = _pace_screen(_sig_fp, _fs_fp, _src_fp, _log,
                                                       options=pace_options)
                            else:
                                _res_fp = {"flag": "unknown", "score": 0.0,
                                           "n_spikes": 0,
                                           "fs_used": _fs_fp if _fs_fp else "",
                                           "fs_source": _src_fp, "method": PACE_METHOD}
                            # Normalise keys the summary expects.
                            meta["pacedetect_flag"] = _res_fp.get("flag", "unknown")
                            meta["pacedetect_score"] = _res_fp.get("score", 0.0)
                            meta["pacedetect_n_spikes"] = _res_fp.get("n_spikes", 0)
                            meta["pacedetect_fs"] = _res_fp.get("fs_used", "")
                            meta["pacedetect_fs_source"] = _res_fp.get("fs_source", "")
                            meta["pacedetect_method"] = _res_fp.get("method", PACE_METHOD)
                            if _res_fp.get("flag") == 1:
                                _log(f"{file_path.name}: pace-screen: likely paced "
                                     f"(score {_res_fp.get('score', 0.0)}, "
                                     f"n={_res_fp.get('n_spikes', 0)}).")
                        else:
                            meta["pacedetect_flag"] = "unknown"
                            meta["pacedetect_score"] = 0.0
                            meta["pacedetect_n_spikes"] = 0
                            meta["pacedetect_fs"] = ""
                            meta["pacedetect_fs_source"] = ""
                            meta["pacedetect_method"] = PACE_METHOD
                        summary_entry = {
                        ...
                        summary_entry.update(_pace_summary_fields(
                            {"flag": meta.get("pacedetect_flag", "unknown"),
                             "score": meta.get("pacedetect_score", 0.0),
                             "n_spikes": meta.get("pacedetect_n_spikes", 0),
                             "fs_used": meta.get("pacedetect_fs", ""),
                             "fs_source": meta.get("pacedetect_fs_source", ""),
                             "method": meta.get("pacedetect_method", PACE_METHOD)}))
                        summary_list.append(summary_entry)
                        # create_dicom_file returns (n_samples, n_leads); transpose
                        # to the standard (12, n) for records.
                    ...
                    # Fast path failed but the standard read may still succeed
                    # (see M3): undo its failure count and fall through instead
                    # of returning failed here.
                    stats.count_failed = failed_before
                    signals, meta = None, {}
                except Exception:
    ...
    if signals is not None:
        ...
        _file_fs = meta.get("fs")  # rate the reader supplied (None for CSV/ASC)
        fs = _file_fs or default_fs
        ...
        if fs:
            meta["fs"] = fs
        # Pacemaker pre-screen on RAW input, before any filtering/resampling
        # (notch/bandpass/wavelet/EMD all attenuate the spike). Rate-less
        # CSV/ASC/MAT use the user-asserted default_fs when present.
        if pace_enabled:
            _fs_src = "file" if _file_fs else ("fallback" if fs else "")
            if fs:
                _res = _pace_screen(np.asarray(signals), fs, _fs_src, _log,
                                    options=pace_options)
            else:
                _res = {"flag": "unknown", "score": 0.0, "n_spikes": 0,
                        "fs_used": "", "fs_source": "", "method": PACE_METHOD}
            meta["pacedetect_flag"] = _res.get("flag", "unknown")
            meta["pacedetect_score"] = _res.get("score", 0.0)
            meta["pacedetect_n_spikes"] = _res.get("n_spikes", 0)
            meta["pacedetect_fs"] = _res.get("fs_used", "")
            meta["pacedetect_fs_source"] = _res.get("fs_source", "")
            meta["pacedetect_method"] = _res.get("method", PACE_METHOD)
            if _res.get("flag") == 1:
                _src_note = (f"@{_res.get('fs_used')} Hz "
                             f"({'asserted rate' if _res.get('fs_source') == 'fallback' else 'file rate'})")
                _log(f"{file_path.name}: pace-screen {_src_note}: likely paced "
                     f"(score {_res.get('score', 0.0)}, n={_res.get('n_spikes', 0)}).")
        else:
            meta["pacedetect_flag"] = "unknown"
            meta["pacedetect_score"] = 0.0
            meta["pacedetect_n_spikes"] = 0
