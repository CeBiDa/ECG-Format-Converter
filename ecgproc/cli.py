...
def build_parser():
    ...
    parser.add_argument("--fs", type=int,
                        help="Sampling rate in Hz to assume for files that do not store one "
                             ...
                             "The pacemaker pre-screen also uses this rate for rate-less files.")
    ...
    pace_group = parser.add_argument_group("Pacemaker pre-screen (raw, before filters)")
    pace_group.add_argument("--no-pacedetect", dest="pace_enabled",
                            action="store_false", default=None,
                            help="Disable the pre-filter pacemaker screen "
                                 "(enabled by default; result goes to pacedetect_* summary columns).")
    pace_group.add_argument("--pace-on-threshold", type=float, default=None,
                            help="Per-lead raw max-|slope| confirmation threshold in uV/ms "
                                 "(default 12.0). Lower finds smaller spikes, higher fewer false alarms.")
    pace_group.add_argument("--pace-min-spikes", type=int, default=None,
                            help="Confirmed spikes needed for flag=1 (default 3; auto-lowered for short records).")
...
def main(argv=None):
    ...
    if args.pace_enabled is not None:
        runner.pace_enabled = args.pace_enabled
    if args.pace_on_threshold is not None:
        runner.pace_options["on_threshold"] = args.pace_on_threshold
    if args.pace_min_spikes is not None:
        runner.pace_options["min_spikes"] = args.pace_min_spikes
    ...
    print(f"Pacemaker pre-screen: {'on (raw, before filters)' if runner.pace_enabled else 'off'}")
