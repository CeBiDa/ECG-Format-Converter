...
def write_dcm(output_path, signals, meta):
    ...
    # Orientation guard: only transpose when the first axis is clearly samples
    # (large) and the second clearly leads (<=16). The old check
    # (shape[0] > shape[1]) transposed short 12-lead segments like (12,10)
    # into (10,12), swapping leads and samples. See C2.
    ...
    # Scale signals to uV if needed. Heuristic (see C1): peak<30 is assumed
    # millivolts. Flat/low uV signals (e.g. 5 uV) are indistinguishable and
    # will be amplified 1000x -- pass meta["units"]="uV" to skip scaling when
    # the unit is known.
    units_hint = str(meta.get("units", "")).strip().lower()
    if units_hint not in ("uv", "microvolt", "microvolts"):
...
def write_hl7(output_path, signals, meta):
    ...
    if signals.size > 0 and np.max(np.abs(signals)) < 30.0:
        ...
        if units_hint not in ("uv", "microvolt", "microvolts"):
            signals = signals * 1000.0
    # Round (not truncate) and map non-finite to 0: bare astype(int) turns
    # inf into INT64_MIN/MAX and truncates 1195.6->1195 (see H2).
...
def write_wfdb(output_path, signals, meta):
    ...
    # Pass meta["units"]="uV" to skip the mV assumption (see C1).
    peak = float(np.max(np.abs(signals))) if signals.size else 0.0
    units_hint = str(meta.get("units", "")).strip().lower()
    if units_hint in ("uv", "microvolt", "microvolts"):
        units, gain = ("uV", 1.0)
    else:
    ...
    d_signal = np.clip(np.round(signals.T * gain), INT16_MIN, INT16_MAX)
