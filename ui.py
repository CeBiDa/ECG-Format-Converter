...
class ProcessWorker(QThread):
    ...
    def run(self):
        try:
            ...
            runner.pace_enabled = s.get("pace_enabled", True)
...
class MainWindow(QMainWindow):
    ...
    def _build_left(self, defaults):
        ...
        g.addLayout(row)
        self.chk_pace = CheckBox("Pacemaker pre-screen (raw, before filters)")
        self.chk_pace.setToolTip(
            "Detect pacing spikes on the raw input before any filter/resample step\n"
            "(bandpass 0.5-40 Hz would remove them). Result goes to the pacedetect_*\n"
            "columns in ecg_summary.csv. Rate-less CSV/ASC files use the asserted\n"
            "\"Input sampling rate\" below. Record-level flag only, not a diagnosis.")
        g.addWidget(self.chk_pace)
        lay.addWidget(box)
        ...
        self.chk_fs.setToolTip(
            ...
            "Also feeds the pacemaker pre-screen for rate-less files."
)
    ...
    def _apply_defaults(self, defaults):
        ...
        self.chk_pace.setChecked(getattr(defaults, "pace_enabled", True))
    ...
    def _refresh_fs_hint(self):
        ...
        if self.cmb_format.currentData() == "wfdb" and not self.chk_fs.isChecked():
            ...
            text = (f"{n} selected file{plural} store{'' if n != 1 else 's'} no sampling rate. "
                    f"{'They' if n != 1 else 'It'} will still be converted, but filtering, "
                    "resampling and the pacemaker pre-screen are skipped without a rate.")
        elif n:
            color = MUTED
            text = (f"{n} selected file{plural} store{'' if n != 1 else 's'} no sampling rate and "
                    f"will be read at {self.cmb_fs.rate()} Hz (filters, resample and pace-screen).")
        else:
            color = MUTED
            text = ("CSV and ASC files never store a sampling rate; filtering, resampling "
                    "and the pacemaker pre-screen need one. Files that carry their own always keep it.")
    ...
    def _gather_settings(self):
        ...
        return {
            ...
            "pace_enabled": self.chk_pace.isChecked(),
    ...
    def _on_start(self):
        ...
        self._append_log(f"Run · format {s['format']} · pipeline {pipe}"
                         ...
                         + (" · pace-screen on" if s.get("pace_enabled", True) else " · pace-screen off"))
