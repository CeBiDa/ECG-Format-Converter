...
from ecgproc.pacedetect import PACE_DEFAULTS, PACE_SUMMARY_COLUMNS, merge_pace_options
from ecgproc.preprocess import EMD_DEFAULTS, PIPELINE_STEPS, merge_emd_options

# Column order of ecg_summary.csv (original 21 + pacemaker pre-screen columns;
# source ``pacemaker`` untouched, detector writes ``pacedetect_*``).
SUMMARY_COLUMNS = [
    "filename",
    "patient_id",
    "patient_firstname",
...
class ExecutionRunner:
    ...
    def __init__(self, config_path=None, path_source=None, path_sink=None):
        ...
        # Pre-filter pacemaker pre-screen; see ecgproc.pacedetect
        self.pace_enabled = True
        self.pace_options = dict(PACE_DEFAULTS)
    ...
    def _load_config(self, config_path):
        ...
        if cfg.has_section("pacedetect"):
            self._load_pace_config(cfg["pacedetect"])
        ...
        # Pacemaker pre-screen default (default ON); per-run flags override.
        if sec.get("pace_enabled", "").strip():
            try:
                self.pace_enabled = sec.getboolean("pace_enabled", self.pace_enabled)
            except ValueError:
                raise ValueError(f"{config_path}: [processing] pace_enabled must be true or "
                                 f"false, got '{sec.get('pace_enabled')}'.") from None
    ...
    def _load_pace_config(self, sec):
        """Read the optional [pacedetect] section, typed after the defaults."""
        for key, default in PACE_DEFAULTS.items():
            raw = sec.get(key, "").strip()
            if not raw:
                continue
            try:
                ...
                raise ValueError(f"config [pacedetect] {key} has invalid value '{raw}'.") from None
            self.pace_options[key] = value
        if "enabled" in self.pace_options:
            self.pace_enabled = bool(self.pace_options["enabled"])
    ...
    def run(self, progress_cb=None, log_cb=None, cancel_cb=None):
        ...
        # Same for the pacemaker pre-screen options.
        if self.pace_enabled:
            self.pace_options = merge_pace_options(self.pace_options)
        ...
        for i, file in enumerate(files):
            ...
            try:
                result = process_file(
                    ...
                    pace_enabled=self.pace_enabled, pace_options=self.pace_options,
                )
            except Exception as e:
